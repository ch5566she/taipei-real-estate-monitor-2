...
def analyze_listing(listing, transactions):
    ...
    prices = [price for _, price in primary_rows]
    reference_prices = [price for _, price in reference_rows]
    sample_count = len(prices)
    normalized_listing_price = listing_net_unit_price(listing)

    comparable_rows = build_comparable_rows(listing, primary_rows, is_reference=False)
    reference_rows_out = build_comparable_rows(listing, reference_rows, is_reference=True)
    ...
    if not prices:
        return {
            ...
            "comparison": {
                ...
                "reference_sample_count": len(reference_prices),
                "market_average": None,
                "market_median": None,
                "q1": None,
                "q3": None,
                "reference_average": round_number(mean(reference_prices)) if reference_prices else None,
                "reference_median": round_number(median(reference_prices)) if reference_prices else None,
...
def build_comparable_rows(listing, rows, is_reference=False):
    ...
    for item, adjusted_price in rows:
...
def build_report(
    ...
    return {
        ...
        "stage": "第25階段：在售物件 × 實價成交多層級比價引擎（A/B/C主要比價＋D行情參考＋E排除）",
...
def main():
    ...
    print("=" * 70)
    print(
        "第25階段：在售物件 × 實價成交多層級比價引擎（A/B/C主要比價＋D行情參考＋Runner 路徑自動搜尋）"
    )
    print("=" * 70)
